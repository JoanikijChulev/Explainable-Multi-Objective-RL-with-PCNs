...
from envs.walkroom.grid import initialize_grid
...
class WalkRoom(gym.Env):
    metadata = {"render_modes": ["ansi"]}

    def __init__(self, size, dimensions=2, seed=0, render_mode=None):
        ...
        self.reward_space = Box(low=np.full((dimensions,), -np.inf, dtype=np.float32),
                                high=np.zeros((dimensions,), dtype=np.float32),
                                dtype=np.float32)
    ...
    def make_boundaries(self, seed=0):
        ...
        d = self.dimensions-1
        room = initialize_grid(self.size, d, seed)
    ...
    def step(self, action):
        ...
        reward = -np.abs(self.actions[action].copy()).astype(np.float32)
        # limit of last coord
        limit = self.room[tuple(pos[:-1].tolist())]
        terminated = bool(pos[-1] >= limit)
        self.pos = pos
        return pos.copy(), reward, terminated, False, {}
